...
inline unsigned long encode_lon_to_x (double longitude) {
    return ((unsigned long) ((longitude + 180) * xconv));
}

inline unsigned long encode_lat_to_y (double latitiude) {
    double S = sin(latitude * torad);
...
inline double decode_x_to_lon (unsigned long X) {
    return ((((double) X) * xcon) – 180.0);
}

inline double decode_y_to_lat (unsigned long Y) {
